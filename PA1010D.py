"""
PA1010D GPS Module Library for MicroPython on Raspberry Pi Pico (I2C Interface)
Author: Assistant
Version: 2.0

This library provides an interface to communicate with the PA1010D GPS module
via I2C and parse NMEA sentences to extract location data.
"""
...
class PA1010D:
    """
    PA1010D GPS module driver for MicroPython using I2C interface
    ...
    def __init__(self, i2c=None, address=0x10, scl_pin=5, sda_pin=4, freq=400000, enable_pin=None):
        """
        Initialize PA1010D GPS module with I2C interface
        
        Args:
            i2c (I2C): Existing I2C object (optional)
            address (int): I2C address of GPS module (default 0x10)
            scl_pin (int): SCL pin number (default 5)
            sda_pin (int): SDA pin number (default 4)
            freq (int): I2C frequency in Hz (default 400kHz)
        ...
        self._buffer = ""
        self.last_sentence = ""
        self._read_size = 255  # Maximum bytes to read per I2C transaction
        
        # Check if device is present
        try:
            self.i2c.scan()
            if address not in self.i2c.scan():
                print(f"Warning: GPS module not found at address 0x{address:02X}")
        except OSError as e:
    ...
    def _read_i2c_data(self):
        """
        Read available data from I2C interface
        
        Returns:
            bytes: Data read from GPS module, or empty bytes if error
        """
        try:
            # Read available data from GPS module
            data = self.i2c.readfrom(self.address, self._read_size)
            
            # Filter out null bytes and non-printable characters except newlines/carriage returns
            filtered_data = bytearray()
            for byte in data:
                if byte == 0:  # Stop at null terminator
                    break
                if byte >= 32 or byte in [10, 13]:  # Printable chars, LF, CR
                    filtered_data.append(byte)
                    
            return bytes(filtered_data)
            
        except OSError as e:
            # I2C error (device not responding, bus error, etc.)
            if hasattr(e, 'errno'):
                if e.errno == 5:  # EIO - No data available
                    return b''
                elif e.errno == 110:  # ETIMEDOUT
                    print("I2C timeout - GPS module may not be ready")
                    return b''
    ...
    def update(self):
        """
        Read and parse available GPS data from I2C interface
        
        Returns:
            bool: True if new data was processed, False otherwise
        """
        # Read available data via I2C
    ...
    def is_connected(self):
        ...
        Returns:
            bool: True if module responds to I2C communication
        """
        try:
    ...
    def get_hdop(self):
        ...
        Returns:
            float: HDOP value or None if unavailable
        """
        return self.hdop
    ...
    def __str__(self):
        ...
        if self.has_fix():
            return (f"GPS ({connection_status}): {self.latitude:.6f}, {self.longitude:.6f} "
                   f"Alt: {self.altitude}m Sats: {self.satellites} HDOP: {self.hdop}")
        else:
            return f"GPS ({connection_status}): No Fix (Sats: {self.satellites or 0})"